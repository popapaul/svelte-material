...
import type { Aggregation, AggregationFn, FilterCondition, GridGroupRow, GridHierarchyRow, GridRow } from "../types";
...
export class DataDataProcessor<TOriginalRow> {
    ...
    */
    ...
    applyGlobalSearch(data: TOriginalRow[]): TOriginalRow[] {
        ...
        const applySimpleSearch = (data: TOriginalRow[]) => {
            ...
            const searchableColumns = flattenColumnStructureAndClearGroups(this.datagrid._columns).filter(c => ['accessor', 'computed'].includes(c.type)).filter(col => col.searchable !== false) as (AccessorColumn<TOriginalRow> | ComputedColumn<TOriginalRow>)[];
            // const searchableColumns = this.datagrid.columns.getFlattenedColumnStructure().filter(c => ['accessor', 'computed'].includes(c.type)).filter(col => col.searchable !== false) as (AccessorColumn<TOriginalRow> | ComputedColumn<TOriginalRow>)[];
    ...
    */
    ...
    private flattenHierarchy(rows: GridRow<TOriginalRow>[]): GridRow<TOriginalRow>[] {
        const flattened: GridRow<TOriginalRow>[] = [];
     ...
     */
    public createBasicRows(rows: TOriginalRow[], parentIndex?: string): GridRow<TOriginalRow>[] {
        return rows.map((row, i) => {
            ...
            return {
                ...
                isGroupRow: (() => false) as any,
                isHierarchyRow: (() => false) as any,
            }
        });
    }
  ...
  */
    ...
    private getVisibleRows(): GridRow<TOriginalRow>[] {
        ...
        if(this.datagrid.features.hierarchy.enabled)
            return this.flattenHierarchy(this.datagrid.cacheManager.rows.filter(x=> "parentIndex" in x && !x.parentIndex));
