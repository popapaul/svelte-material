...
import { HierarchyFeature } from "./hierarchy.svelte";
...
export class DatagridFeatures<TOriginalRow = any> {
    ...
    /**
     * Hierarchy feature for the DataGrid, enabling parent/children  rows.
     */
    ...
    constructor(datagrid: DatagridCore<any>, config?: DatagridCoreConfig<TOriginalRow>) {
        ...
        this.hierarchy = new (config?.features?.hierarchy || HierarchyFeature)(this.datagrid, config?.initialState?.hierarchy || {});
