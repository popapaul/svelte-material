...
export function createAccessorColumn<
  ...
  TMeta,
>(
  { header, accessorKey, columnId, accessorFn: getValue, align, options, _meta, state, ...rest }: CreateAccessorColumnProps<TOriginalRow, TKey, TMeta>
  ...
  const accessorFn: (row: TOriginalRow) => any =
