...
<script lang="ts" generics="ItemType">
	...
	function getAvgSize() {
		...
		console.log('sizeArr',  sizeArr.reduce((a, b) => a + b, 0) / sizeArr.length);
...
<div
	...
	style={listStyle}
>
	<div bind:this={listInner} class="grid-container !w-full min-w-max {className}" style={listInnerStyle}>
		{@render header?.()}
		{#each visibleItemsInfo as item(item.item.identifier)}
			<div class="grid-body" animate:flip={{duration:200}} in:fade={{duration:200}}>
			{@render vl_slot(item)}
		</div>
		{/each}
		{@render footer?.()}
