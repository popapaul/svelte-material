<script context="module" lang="ts">
  ...
  export type DataGridContext<T=any>= ReturnType<typeof createContext<T>>;
    ...
    const _context = writable({data, columns: {} as Record<string,Column<T>>,  hasChildren:null as(item:T)=>boolean, getChildren: null as (item:T)=>Promise<T[]>});
            ...
            if(!multipleSort)
              ...
              switch(columns[sortBy].direction){
                ...
                case "desc":
                  direction = "asc"; break;
                default:
                  direction = reverse ? "desc" : "asc"; break;
              }
