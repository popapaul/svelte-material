<script lang="ts">
	...
	if (isHead && sortBy) sortBy = context?.registerColumn({ direction: sort, sortBy, compare });
...
{#if isHead}
	<TableCell
		sorted={$context.columns?.[sortBy]?.direction}
		class="s-table-sorter {klass}"
		{width}
		{...$$restProps}
		on:click={handleSort}
	>
		<div style="display:flex; gap:4px;" slot="header">
			...
			{#if sortBy}
				<Icon
					class="s-table-sorter-icon"
					size="23"
					rotate={$context.columns?.[sortBy]?.direction == 'asc' ? 0 : 180}
					path={icons.arrow_up}
				/>
			{/if}
		</div>
