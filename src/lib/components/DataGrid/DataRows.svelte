<script lang="ts">
    ...
    const sort=(items:T[])=>{
        ...
        return  items.sort((a,b)=>{
            ...
            const propriety = column.propriety || ((item:T)=>sortBy.split(".").reduce((acc,key)=>acc[key],item));
            const compare = column.compare || ((a,b)=> String(propriety(a)).localeCompare(String(propriety(b))));
...
{#each sort(data) as context,index}
   {@const anyChildren = hasChildren?.(context)}
    <DataRow item={context} expand={ expand ||  !!hasChildren} on:expand={()=> anyChildren && handleExpand(context)}>
        <slot {index} {context} />
