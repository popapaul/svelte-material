<script lang="ts">
  ...
  function handleMove(event) {
   if (!tracked) return;
    let [mouseX,mouseY] = event.type=="mousemove" ? [event.clientX,event.clientY] : [event.touches[0].clientX, event.touches[0].clientY];
  ...
  function handleDown(event) {
      ...
      const [offsetX,offsetY] = event.type=="mousedown" ? [event.offsetX,event.offsetY] : [event.targetTouches[0].clientX - rect.left, event.targetTouches[0].clientY - rect.top];
