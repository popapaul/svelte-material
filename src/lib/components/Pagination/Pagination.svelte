<script lang="ts">
	...
	export let count = 0;
	export let pageSizes: number[] = Array.from(new Set([pageSize, 10, 25, 50, 75, 100, 150])).sort(
		(a, b) => a - b
	);
	export let page = 0;
	...
	const calcDisplacer = (page: number) => {
		if (page - middle < 0) return Math.abs(0 - (page - middle));
		if (Number(page) + middle > pageCount - 1) return pageCount - 1 - (Number(page) + middle);
		return 0;
	};
...
<div class="s-pagination {klass}">
	...
	<div class="flex gap-2">
		<Button fab size="x-small" on:click={(e) => onChange(e, 0)} disabled={page == 0}>
			<Icon path={icons.arrow_left_double} />
		</Button>
		<Button fab size="x-small" on:click={(e) => onChange(e, page - 1)} disabled={page == 0}>
		...
		<Button fab size="x-small" on:click={(e) => onChange(e, lastPage)} disabled={page == lastPage}>
