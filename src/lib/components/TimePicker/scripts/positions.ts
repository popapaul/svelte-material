...
/* Gets the new date computed with the hours position.
 (0, 0) = center, 0deg = 3h */
const getDateFromHoursPosition = (date:Date, position:Postion, width:number, is24h:boolean, isAm:boolean) => {
    /* Retrieves the hour */
...
/* Gets the new date computed with the minutes position.
 (0, 0) = center, 0deg = 15m */
const getDateFromMinutesPosition = (date:Date, position:Postion, minutesIncrement:string|number) => {
