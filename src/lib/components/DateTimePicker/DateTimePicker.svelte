<script lang="ts">
  ...
  const handleDate = ({detail:newDate}:CustomEvent<Date>)=>{
      value ??= new Date();
      value = new Date(newDate.getFullYear(), newDate.getMonth(), newDate.getDate(), value.getHours(), value.getMinutes());
  ...
  const handleHour= ({detail:newTime}:CustomEvent<Date>)=>{
      value ??= new Date();
      value = new Date(value.getFullYear(), value.getMonth(), value.getDate(), newTime.getHours(), hourOnly ? newTime.getMinutes() : 0);
      dispatch("change", value);
      if(hourOnly)
...
<div class="datepicker">
  <!-- svelte-ignore a11y-click-events-have-key-events -->
    ...
    <div class="body">
      {#if mode === "days" || mode === "month" || mode === "year" }
        <DatePicker {...$$restProps} bind:mode {locale} {value} header={false} on:change={handleDate}/>
        {:else}
        <TimePicker {...$$restProps} hasToolbar={false} bind:mode {value} on:change  on:hour={handleHour} on:minute={handleMinutes}></TimePicker>
      {/if}
    </div>
  </div>
