...
export const reposition = (
	...
	const { container, margin, padding, position, variantFlipOrder, positionFlipOrder } = {
...
export const createPopper: NanoPopConstructor = (
	...
	// Resolve options
	const baseOptions: Partial<NanoPopOptions> =
		typeof reference === 'object' && !(reference instanceof HTMLElement)
			? reference
			: { reference, popper, ...options };

	const update = () => reposition(baseOptions.reference, baseOptions.popper, baseOptions);
